...
public interface MachineRepository extends JpaRepository<Machine, Integer> {
    ...
    // "AND k.year = (SELECT TOP 1 k2.year FROM machine_kpi k2 WHERE k2.group_id =
    // :groupId ORDER BY CASE WHEN k2.year = :year AND k2.month = :month THEN 0 ELSE
    // 1 END, k2.year DESC, k2.month DESC) "
    // +
    // "AND k.month = (SELECT TOP 1 k2.month FROM machine_kpi k2 WHERE k2.group_id =
    // :groupId ORDER BY CASE WHEN k2.year = :year AND k2.month = :month THEN 0 ELSE
    // 1 END, k2.year DESC, k2.month DESC)", nativeQuery = true)
    // List<Machine> findMachinesByGroupIdLatestOrCurrent(
    // @Param("groupId") String groupId,
    // @Param("year") int year,
    // @Param("month") int month);

    // List<Machine> findByGroup_GroupIdAndYearAndMonth(String groupId, int year,
    // int month);
